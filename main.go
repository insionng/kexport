...
Usage:
  kexport [-contexts=""] [-defctx=false] [-convnew=true] [-skiperrimpl=true] [-outpath="./extension"] packages
...
func init() {
	...
	flag.StringVar(&flagExportPath, "outpath", "./extension", "optional set export root path")
}
...
func toLowerCaseStyle(s string, b bool) string {
	if b {
		if len(s) <= 1 {
			return s
		}
